...
let phraseCharacter: number = 0;

// Holds the handle returned from setInterval
let intervalValue: number;

const element: HTMLElement = document.querySelector("#text");
const cursor: HTMLElement = document.querySelector("#cursor");

// Implements typing effect
function Type() {
  // Get substring with 1 character added
  let text: string = thingsILike[phrase].substring(0, phraseCharacter + 1);
...
function Delete() {
  ...
  if (text === "") {
    ...
    if (phrase == thingsILike.length - 1) phrase = 0;
    else phrase++;
